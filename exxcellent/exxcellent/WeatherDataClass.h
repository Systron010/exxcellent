...
private:
	struct dayData { // Store different values for one day
		...
		float mn;
		float rAvSLP;
		bool operator<(const dayData& other) // <Operator to search minimum in temperature spread (mxt-mnT) using 'std::min_element()'
		{
	...
	vector<dayData>::iterator minTempSpreadDay; // Data of the day with the smallest temperature spread (This is the solution of the weather challenge)
